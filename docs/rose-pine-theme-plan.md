...
home/desktop/
  ...
  config/kwm-config.zon ← KWM 模板：@placeholder@ 占位符
  config/rose-pine-firefox-userChrome.css ← Firefox Chrome UI 模板：@placeholder@ 占位符
  config/rose-pine-firefox-userContent.css ← Firefox about: 页面模板：@placeholder@ 占位符
  config/gtk-tooltip-dark.css ← GTK tooltip dark（Firefox NAC tooltip）
  config/gtk-tooltip-light.css ← GTK tooltip light
  config/dark-reader-settings.json ← Dark Reader 扩展配置
  config/rose-pine-yazi-flavor.toml ← Yazi flavor 模板：@placeholder@ 占位符
  config/rose-pine-yazi-tmtheme.xml ← Yazi 语法高亮模板：@placeholder@ 占位符
...
packages/
  ...
  trae-bootstrap.cjs   ← Darkman 集成 + Wayland 检测
