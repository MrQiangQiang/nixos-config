...
home/desktop/
  ...
  config/kwm-config.zon  ← KWM 模板：@placeholder@ 占位符（palette 色值）
  config/rose-pine-bat.tmTheme ← bat 语法高亮模板：@placeholder@ 占位符
...
### Fix 1: wob.nix — 添加 HM systemd 服务 ✅

wob 原来只有 `home.packages` 安装包，没有 systemd 服务，darkman 中 `systemctl --user restart wob` 静默失败。

修复：手写 `systemd.user.services.wob`（不用 `services.wob`），settings 留空。

**`settings` 必须留空**。如果设置了 settings，home-manager 会生成 `~/.config/wob/wob.ini` 为只读符号链接，且 wob 启动时带 `--config /nix/store/...` 参数，darkman 复制的配置会被完全忽略。

当 `settings` 留空时：
- `xdg.configFile."wob/wob.ini"` 不会被创建
- wob 启动时不带 `--config` 参数，从 `~/.config/wob/wob.ini` 读取
- darkman 可以自由 ln -sf 配置到该路径，无冲突

### Fix 2: kwm-config.zon — 添加音量键+亮度键绑定 ✅

与 Fix 1 关联。wob 可用后，音量键和亮度键通过 spawn + 独立脚本向 FIFO 写入值：

- `XF86_AudioRaiseVolume` / `XF86_AudioLowerVolume`：spawn volume-up/down → wob.sock
- `XF86_AudioMute`：spawn volume-mute → wob.sock
- `XF86_MonBrightnessUp` / `XF86_MonBrightnessDown`：spawn brightness-up/down → wob.sock

独立脚本由 media-keys.nix 提供（writeShellScriptBin），brightnessctl 安装在 wob.nix 的 `home.packages` 中。

### Fix 3: swayidle.nix — 添加 wlopm 关屏（含 resumeCommand）✅
...
**`resumeCommand` 是必须的**，不是可选优化。wlopm `--off` 关闭显示器后，swayidle 检测到活动但没有 resume 命令执行 `wlopm --on`，屏幕会一直黑屏。

wlopm 只关闭显示输出，不影响 CPU/内存/网络/SSH/音频，不会触发休眠。

### Fix 4: gammastep → wlsunset ✅

wlsunset 是原生 Wayland 应用，通过 `wlr-gamma-control-unstable-v1` 协议工作。gammastep 虽然在 River 上也能工作（同样使用 wlr-gamma-control 协议），但 wlsunset 更符合 do one thing well——你只需要手动经纬度 + 日/夜温度，wlsunset 完全覆盖，代码量小一个数量级。

文件重命名：`gammastep.nix` → `wlsunset.nix`，default.nix imports 同步更新。

### Fix 5: 图标主题 rose-pine-icons → Papirus ✅

`rose-pine-gtk-theme` 的 `share/icons/` 为空，不提供任何图标主题。原配置 `iconTheme.name = "rose-pine-icons"` 是错误配置——GTK 找不到该主题，回退到 hicolor（只有极少数图标），导致 fuzzel 中大部分应用无图标。

修复：`iconTheme.name = "Papirus"`，fuzzel 通过 `config.gtk.iconTheme.name` 引用（唯一来源，改主题只改 gtk.nix 一处）。

为什么选 Papirus：社区最流行的 SVG 图标主题之一，覆盖率高，继承 hicolor 作为 fallback。Papirus 不提供的图标（darkman、yazi 等自定义应用），通过 hicolor fallback 机制补充。

### Fix 6: fuzzel 图标缺失 ✅

fuzzel 启动器中 5 个应用无图标，根因各不相同：

| 应用 | 根因 | 修复 |
|------|------|------|
| Trae CN | `find` 匹配到 383 字节假 SVG，安装为 `.png` 后缀 → PNG 加载器读 SVG 失败 | 直接引用 `resources/app/resources/linux/code.png` |
| Toggle darkman | desktopEntry 的 `Icon=` 未设置 | darkman.nix 添加 `xdg.desktopEntries.darkman` + `darkman.svg` |
| Yazi File Manager | yazi 包不提供图标 | filemanager.nix 添加 `yazi.svg` |
| 键盘布局测试器 | Papirus 将 `input-keyboard` 放在 `devices` 上下文，fuzzel 只搜索 `applications`/`apps`/`legacy` | im.nix 在 `scalable/apps` 中提供 `input-keyboard.svg` |
| NixOS Manual | nixos-manual 包自带图标，Papirus 覆盖 | 无需修复 |

图标归属遵循 co-location 原则（见"图标管理原则"章节）。

## 不需要修复

| # | 原计划问题 | 为什么不需要修复 |
|---|---|---|
| 1 | startup_cmds 应为空 | KWM 的 `posix.execve` 搜索 PATH，短名称命令可正确执行；与 KWM 生命周期绑定的轻量服务留在 startup_cmds 是合理的 |
| 2 | river/init 应启动 polkit-gnome/wl-paste | 已由 startup_cmds 启动，重复启动反而有问题 |
| 3 | 截图应保存文件 | 剪贴板方式简洁有效，日常使用足够 |
| 4 | fcitx5.nix 缺失 | 系统级 modules/im.nix 已完整配置，river/init 中启动守护进程，不需要 home-manager 层重复 |
| 5 | default.nix 缺 fcitx5 导入 | 同上 |
