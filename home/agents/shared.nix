...
let
  ...
  readContent = type: name:
    builtins.readFile (./shared + "/${type}/${name}.md");
  ...
  ## 构建各工具消费的 attrset
  commands = lib.mapAttrs (name: meta:
    mkFrontmatter { inherit (meta) description; } + readContent "commands" name
  ) commandMeta;

  skills = lib.mapAttrs (name: meta:
    mkFrontmatter { inherit (meta) name description; } + readContent "skills" name
  ) skillMeta;

  agents = lib.mapAttrs (name: meta:
    mkFrontmatter { inherit (meta) name description; } + readContent "agents" name
  ) agentMeta;

  rules = builtins.listToAttrs (map (name:
    lib.nameValuePair name (readContent "rules" name)
  ) ruleNames);

  references = builtins.listToAttrs (map (name:
    lib.nameValuePair name (readContent "references" name)
  ) referenceNames);

  ## 合并所有 rules（供 context 选项使用）
  combinedRules = lib.concatStringsSep "\n\n---\n\n" (builtins.attrValues rules);
in {
  inherit commands skills agents rules references combinedRules;
}
