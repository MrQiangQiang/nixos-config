...
let
  ...
  crossConflicts = lib.flatten (lib.mapAttrsToList (app: data:
    lib.concatMap (b:
      let bid = "${formatMods b.mods}+${b.key}";
          matches = lib.filter (kid: kid == bid) kwmIds;
      in if matches != [] then
        ["${app}: ${bid} → ${b.desc} conflicts with kwm global binding"]
      else []
    ) data.bindings
  ) documentedApps);
...
lib.mkIf isDesktopEnabled {
  ...
  warnings = lib.optional (realConflicts != [])
