...
let
  kwnSrc = pkgs.fetchFromGitHub {
    ...
    repo = "kwm";
    rev = "main";
    hash = "sha256-AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA";
  };
  ...
  in {
   # nixosModules.river-kwm = { config, pkgs, ... }: {
      ...
      home-manager.users."a" = { config, pkgs, ... }: {
	  ...
	  text = ''
	    ...
	    ${pkgs.rivertile}/bin/rivertile -view-padding 6 -outer-padding 6 &
            ...
            ${pkgs.river}/bin/riverctl map normal Super J focus-view next
            ${pkgs.river}/bin/riverctl map normal Super K focus-view previous
